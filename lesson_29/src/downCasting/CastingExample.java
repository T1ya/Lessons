...
public class CastingExample {
    public static void main(String[] args) {
        ...
        //нисходящее преобразование (down casting)
        int intX = 10;
        double doubleX = intX;  //неявное автоматическое нисходящее преобразование
        System.out.println(doubleX);

        intX = (int)doubleX;//явное нисходящее преобразование
        System.out.println(intX);

        // зачем? Например нужно вызвать дочерний метод
        Animal animalD = new Dog();
        // animalD.bark(); // это не работает!
        Dog newDog = (Dog) animalD; // явно преобразуем обратно к классу Dog с помощью down casting
        newDog.bark(); // a теперь работает!
        // ВАЖНО
        // явное автоматическое нисходящее преобразование - метод небезопасный.
        // При попытке downcast преобразовать не к тому классу вылетит ошибка компиляции
        // Dog dog2 = (Dog) cat;
        // dog2.bark(); // error

        // можно проверить тип объекта для избежания объектов перед downcasting с помощью instanceof
        for (Animal a: animals) {
            if (a instanceof Cat) {
                a.voice();
